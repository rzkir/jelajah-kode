import { ArticlesCategory } from "@/models/ArticlesCategory";

import { connectMongoDB } from "@/lib/mongodb";

import { generateFrameworkId } from "@/hooks/TextFormatter";

export async function createArticlesCategory(data: {
  title: string;
  categoryId?: string;
}) {
  try {
    await connectMongoDB();
    // Create category with the provided categoryId or generate from title
    const categoryId = data.categoryId || generateFrameworkId(data.title);
    const category = await ArticlesCategory.create({
      title: data.title,
      categoryId: categoryId,
    });
    return category;
  } catch (error) {
    throw error;
  }
}

// Get all categories
export async function getAllArticlesCategories() {
  try {
    await connectMongoDB();
    const categories = await ArticlesCategory.find()
      .sort({ createdAt: -1 })
      .lean();
    return categories;
  } catch (error) {
    throw error;
  }
}

// Get a single category by ID
export async function getArticlesCategoryById(id: string) {
  try {
    await connectMongoDB();
    const category = await ArticlesCategory.findById(id);
    return category;
  } catch (error) {
    throw error;
  }
}

// Update a category
export async function updateArticlesCategory(
  id: string,
  data: { title: string; categoryId?: string }
) {
  try {
    await connectMongoDB();
    // Generate categoryId from title if not provided
    const categoryId = data.categoryId || generateFrameworkId(data.title);
    const category = await ArticlesCategory.findByIdAndUpdate(
      id,
      { title: data.title, categoryId: categoryId, updatedAt: new Date() },
      { new: true }
    );
    return category;
  } catch (error) {
    throw error;
  }
}

// Delete a category
export async function deleteArticlesCategory(id: string) {
  try {
    await connectMongoDB();
    const category = await ArticlesCategory.findByIdAndDelete(id);
    return category;
  } catch (error) {
    throw error;
  }
}

// Get only category names
export async function getArticlesCategoryNames() {
  try {
    await connectMongoDB();
    const categories = await ArticlesCategory.find().select("title -_id");
    return categories.map((category: { title: string }) => category.title);
  } catch (error) {
    throw error;
  }
}
