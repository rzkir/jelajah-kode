...
export async function GET(request: NextRequest) {
  try {
    ...
    const userId = searchParams.get("userId"); // For getting all user reviews

    // If userId is provided and matches current user, return all their reviews
    if (userId === "me" || userId === user._id.toString()) {
      ...
      return NextResponse.json({
        reviews: ratingsWithProducts,
      });
    }

    // If productsId is provided, check if user has rated this product
    if (!productsId) {
      return NextResponse.json(
        { error: "productsId or userId is required" },
        { status: 400 }
      );
    }

    // If both productsId and userId are provided, check if that specific user has rated
    // Allow admins to check any user's rating, or users to check their own
    let targetUserId = user._id.toString();
    if (userId && userId !== "me") {
      // Only allow admins to check other users' ratings, or users to check their own
      if (user.role === "admins" || userId === user._id.toString()) {
        targetUserId = userId;
      }
    }

    // Check if target user has already rated this product
    const existingRating = await Ratings.findOne({
      productsId,
      "author._id": targetUserId,
    });
